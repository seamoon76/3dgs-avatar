############################################################
# This code is from ARAH's code, we use it to evaluate mesh quality under ARAH's protocol
# Reference:
#  https://github.com/taconite/arah-release/issues/9#issuecomment-1359209138
##############################################################
import trimesh
import torch
import json
...
def main():
    parser = argparse.ArgumentParser(description="Run the script with subject argument")
    parser.add_argument('--subject', type=str, required=True, help='Subject ID, e.g., 386')
    ...
    print("Chamfer Distance: {}".format(torch.stack(nb_losses, dim=0).mean().item() / 2.0))
    
    print("Normal Consistency: {}".format(np.mean(nb_nc)))
