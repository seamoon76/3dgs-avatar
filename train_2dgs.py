...
def training(config):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        lambda_prior_normal = opt.lambda_prior_normal if iteration > 7000 else 0.0
        ...
        # add prior normal
        prior_normal = data.prior_normal.cuda()
        cos_sim = torch.nn.functional.cosine_similarity(surf_normal, prior_normal, dim=0)
        prior_normal_loss = lambda_prior_normal * ((1 - cos_sim) * gt_mask).mean()

        # loss
        loss = loss + dist_loss + normal_loss + prior_normal_loss
        ...
        with torch.no_grad():
            ...
            log_loss = {
                ...
                'loss/normal_loss': normal_loss.item(),
                'loss/prior_normal_loss': prior_normal_loss.item(),
    ...
    for config in validation_configs:
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, data_idx in enumerate(config['cameras']):
                ...
                norm = depth.max()
                depth = depth / norm
                depth_image = colormap(depth.cpu().numpy()[0], cmap='turbo')
                normal_image = render_pkg["rend_normal"] * 0.5 + 0.5
                ...
                wandb_img = wandb.Image(depth_image[None], caption=config['name'] + "_view_{}/depth".format(
                    data.image_name))
                examples.append(wandb_img)
                wandb_img = wandb.Image(normal_image[None], caption=config['name'] + "_view_{}/render_normal".format(
                    data.image_name))
                examples.append(wandb_img)
                ...
                try:
                    ...
                    wandb_img = wandb.Image(alpha_image[None], caption=config['name'] + "_view_{}/render_alpha".format(
                    data.image_name))
                    examples.append(wandb_img)
                    wandb_img = wandb.Image(surf_normal[None], caption=config['name'] + "_view_{}/surf_normal".format(
                        data.image_name))
                    examples.append(wandb_img)

                    rend_dist = render_pkg["rend_dist"]
                    rend_dist = colormap(rend_dist.cpu().numpy()[0])
                    wandb_img = wandb.Image(rend_dist[None], caption=config['name'] + "_view_{}/render_dist".format(
                        data.image_name))
                    examples.append(wandb_img)
                except:
                    pass
